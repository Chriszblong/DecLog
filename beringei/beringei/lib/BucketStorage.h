...
class BucketStorage {
 public:
  ...
  // 给某个Bucket内某范围内的数据点上锁
  bool lockDataPointInBucket(
      int64_t begin,
      int64_t end,
      uint32_t position,
      BucketStorageId id);

  // 给某个Bucket内某范围内的数据点解锁
  bool unlockDataPointInBucket(
      int64_t begin,
      int64_t end,
      uint32_t position,
      BucketStorageId id);
  ...
  // written by yls
  // 获取buckets
  BucketData* getBuckets() const{
    return data_.get();
  }

  // written by yls
  // 获取newestPosition
  int getNewestPosition() const{
    return newestPosition_;
  }
