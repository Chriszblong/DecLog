...
class ClientThread extends Thread
{
     ...
     */
    public ClientThread(DB db, boolean dotransactions, Workload workload, Properties props, int opcount, double targetperthreadperms, CountDownLatch completeLatch, int threadid, int threadcount)
    {
        _db=db;
...
public class Client
{
    ...
    /**
     * Passing the phase which we ware executing (load/run) to the client
     * Some clients need that (druid e.g.)
    ...
    public static void main(String[] args)
    {
        ...
        while (args[argindex].startsWith("-"))
        {
            ...
            {
                props.setProperty(PHASE_PROPERTY, "load");
                dotransactions=false;
                argindex++;
            }
            else if (args[argindex].compareTo("-t")==0)
            {
                props.setProperty(PHASE_PROPERTY, "run");
                dotransactions=true;
                argindex++;
            }
