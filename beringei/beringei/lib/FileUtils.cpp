...
FileUtils::FileUtils(
    ...
    : directory_(dataDirectory) {
    prefix_ = "shard-" + std::to_string(shardId) + "-" + prefix;
...
boost::filesystem::path FileUtils::getPath(int64_t id) {
  boost::filesystem::path path = filePath(id);
  LOG(INFO) << "Opening file: " << path.c_str();
  return path;
}

boost::filesystem::path FileUtils::getPath(int64_t id, const int threadNum) {
  boost::filesystem::path path = filePath(id);
  // path += "-" + std::to_string(threadNum);
  path += std::to_string(threadNum);
  LOG(INFO) << "Opening file: " << path.c_str();
  return path;
}
...
// int
// FileUtils::open(int64_t id) {
//   boost::filesystem::path path = filePath(id);
//   LOG(INFO) << "Opening file: " << path.c_str();
//   int fd;
//   if ((fd = open((const char*)path.c_str(), O_RDWR)) < 0) {
// 		PLOG(ERROR) << "Failed to open file: " << path.c_str();
//     GorillaStatsManager::addStatValue(kFileOpenFailures);
// 	}
