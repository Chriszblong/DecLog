...
int main(int argc, char** argv) {
  ...
  FLAGS_start_time = std::atoi(argv[2]);

  FLAGS_end_time = std::atoi(argv[3]);
  ...
  LOG(INFO) << "The key Name is: " << keyName;

  if (FLAGS_shard_id == -1) {
    FLAGS_shard_id = std::hash<std::string>()(keyName) % shardCount;
  }

  LOG(INFO) << "The shard_id calculated by hash map is: " << FLAGS_shard_id;

  FLAGS_shard_id = std::atoi(argv[4]);

  LOG(INFO) << "The args of shard_id: " << FLAGS_shard_id;
