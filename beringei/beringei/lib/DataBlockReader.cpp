...
DataBlockReader::DataBlockReader(
    ...
    : dataFiles_(shardId, BucketStorage::kDataPrefix, dataDirectory),
      completedFiles_(shardId, BucketStorage::kDataPrefix, dataDirectory) {}
...
std::set<int32_t> DataBlockReader::findCompletedBlockFiles() {
  const std::vector<int64_t> files = completedFiles_.ls();
  std::set<int32_t> completedBlockFiles(files.begin(), files.end());
