...
class TimeSeries {
 public:
  ...
  static void getValues(
    ...
    TimeSeriesStream::readValues(values, block.data, block.count, begin, end, updatedDataMap);
  }

  // Create the updatedDataMap.
  static void createMap(
      const TimeSeriesBlock& block,
      std::map<int64_t, double>* updatedDataMap) {
    TimeSeriesStream::createUpdatedDataMap(block.data, block.count, updatedDataMap);
  }
};
}
} // facebook::gorilla
