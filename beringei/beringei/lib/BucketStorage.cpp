...
extern const uint32_t kMaxPageCount = 262144;
...
extern const size_t kLargeFileBuffer = 1024 * 1024;
...
char* BucketStorage::findDataPointInBucket(
    const TimeValuePair& value,
    const uint32_t& position,
    const BucketStorageId& id,
  ...
  if (pageIndex < data_[bucket].pages.size() &&
      data_[bucket].pages[pageIndex]) {
    targetBlock = data_[bucket].pages[pageIndex]->data + pageOffset;
    tombstoneBitID = TimeSeriesStream::findDataPointInStream(
        data_[bucket].pages[pageIndex]->data + pageOffset,
        value,
        itemCount,
        targetTimestamp);
  }
...
bool BucketStorage::lockDataPointInBucket(
    int64_t begin,
    int64_t end,
    uint32_t position,
    BucketStorageId id) {
  ...
  if (pageIndex < data_[bucket].pages.size() &&
      data_[bucket].pages[pageIndex]) {
    return TimeSeriesStream::lockInExtension(
        data_[bucket].pages[pageIndex]->data + pageOffset,
        dataLength,
        itemCount,
        begin,
...
bool BucketStorage::unlockDataPointInBucket(
    int64_t begin,
    int64_t end,
    uint32_t position,
    BucketStorageId id) {
  ...
  if (pageIndex < data_[bucket].pages.size() &&
      data_[bucket].pages[pageIndex]) {
    return TimeSeriesStream::unlockInExtension(
        data_[bucket].pages[pageIndex]->data + pageOffset,
        dataLength,
        itemCount,
        begin,
...
bool BucketStorage::loadPosition(
  ...
  if (position == -1) {
    LOG(INFO) << "This is a stream file. " << std::endl;
    return true;
  }
...
void BucketStorage::finalizeBuckets() {

  for(int i = 0;i<numBuckets_;i++){
    //std::lock_guard<std::mutex> guard(data_[i].pagesMutex);
    if(data_[i].finalized || data_[i].disabled || data_[i].position % numBuckets_ != i)
      continue;
    data_[i].finalized = true;
    if (data_[i].activePages > 0 && data_[i].timeSeriesIds.size() > 0) {
      write(data_[i].position, data_[i].pages, data_[i].activePages, data_[i].timeSeriesIds, data_[i].storageIds);
    }
  }
