 ...
 * items from min to max inclusive). After you construct the instance, you can change the number of items by calling nextLong(itemcount) or nextLong(itemcount).
 * <p/>
...
*/
    ...
    public ScrambledZipfianGenerator(long min, long max, double _zipfianconstant, double zeta) {
        _min = min;
        _max = max;
        _itemcount = _max - _min + 1;
        if (_zipfianconstant == USED_ZIPFIAN_CONSTANT) {
            ...
            gen = new ZipfianGenerator(0, ITEM_COUNT, _zipfianconstant, zeta);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        ScrambledZipfianGenerator gen = new ScrambledZipfianGenerator(0,24,0.9);
