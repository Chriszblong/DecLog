...
void BeringeiClientImpl::get(
  ...
  for (int i = 0; i < gorillaResult.results.size(); i++) {
    ...
    auto& mapBlock = gorillaResult.results[i].data[0];
    ...
    // Get values from the remaining blocks.
    for (int j = 1; j < gorillaResult.results[i].data.size(); ++j) {
    // for (auto& block = : gorillaResult.results[i].data) {
      auto& block = gorillaResult.results[i].data[j];
      TimeSeries::getValues(
          block, result[i].second, request.begin, request.end, &updatedDataMap);
    }
  }
}
