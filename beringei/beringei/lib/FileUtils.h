...
class FileUtils {
 public:
  ...
  // // This is the NVM version of open().
  // // Get the file with the given id.
  // // Does fully-buffered IO with a buffer of the given size or unbuffered
  // // (direct) IO if bufferSize is 0.
  // // Returns nullptr on failure.
  ...
  // Get the file path for NVM.
  boost::filesystem::path getPath(int64_t id);
  boost::filesystem::path getPath(int64_t id, const int threadNum);
